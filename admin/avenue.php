...
if (isset($_POST['btn_submit'])) {
  ...
  $reque=$db->prepare("INSERT INTO avenue (nom_avenue,ref_quartier) VALUES (:nom_avenue,:ref_quartier) ");
...
if (isset($_POST['submit'])) {
    extract($_POST);
    $id_avenus = htmlspecialchars($_POST['id_avenus']);
    $nom_avenue = htmlspecialchars($_POST['nom_avenue']);
    $ref_quartier = htmlspecialchars($_POST['ref_quartier']);
    

  $req=$db->prepare("UPDATE avenue SET nom_avenue=:nom_avenue,ref_quartier=:ref_quartier WHERE id_avenus=:id_avenus");
...
<tbody>
<?php $requete=$db->query("SELECT * FROM avenue INNER JOIN quartier ON avenue.ref_quartier=quartier.id_quartier"); 
...
<div class="modal fade" id="large-Modal<?= $g['id_avenus']; ?>" tabindex="-1" role="dialog">
<div class="modal-dialog modal-lg" role="document">
<div class="modal-content">
<div class="modal-header">
<h4 class="modal-title">Modifier l'avanue <?= $g['nom_avenue']; ?> </h4>
...
<div class="modal-body">
   <form action="" method="post">
    <input type="hidden" name="id_avenus" value="<?= $g['id_avenus']; ?>">

 
  <div class="row">
  <div class="col-md-6">
    <?php $repp=$db->query("SELECT * FROM quartier"); ?>
        <select class="form-control" name="ref_quartier" id="">
                                    <?php while ($ggg = $repp->fetch()) { ?>
                                <option value="<?= $ggg['id_quartier'];?>"><?= $ggg['nom_quartier'];?> </option>
                                <?php } ?>
...
<div class="modal-content">
<div class="modal-header">
<h4 class="modal-title">Suppression de l'avenue <?= $g['nom_avenue']; ?> </h4>
<button type="button" class="close" data-dismiss="modal" aria-label="Close">
<span aria-hidden="true">&times;</span>
</button>
</div>
<div class="modal-body">
  ...
  <div class="row">
     ...
     <div class="col-md-12">
        <h4>Etes vous sur de vouloir suppeimer <?= $g['nom_avenue']; ?>?</h4>
     </div>
...
<td><?= $g['nom_avenue']; ?></td>
<td><?= $g['created_avenu']; ?></td>


<td>
<button type="button" class="btn btn-warning waves-effect" data-toggle="modal" data-target="#large-Modal<?= $g['id_avenus']; ?>"><i class="feather icon-edit"></i> </button>
<button type="button" class="btn btn-danger waves-effect" data-toggle="modal" data-target="#large-Mod<?= $g['id_avenus']; ?>"><i class="feather icon-trash"></i> </button>
</td>
