...
if (isset($_POST['btn_submit'])) {
  ...
  $reque=$db->prepare("INSERT INTO quartier (nom_quartier,ref_commune) VALUES (:nom_quartier,:ref_commune) ");
  ...
  if ($result) {
     ...
     Swal.fire({
      position: 'top-end',
      icon: 'success',
      title: 'quartier inserer avec success',
...
if (isset($_POST['submit'])) {
    extract($_POST);
    $id_quartier = htmlspecialchars($_POST['id_quartier']);
    $nom_quartier = htmlspecialchars($_POST['nom_quartier']);
    $ref_commune = htmlspecialchars($_POST['ref_commune']);
    

  $req=$db->prepare("UPDATE quartier SET nom_quartier=:nom_quartier,ref_commune=:ref_commune WHERE id_quartier=:id_quartier");
...
if (isset($_POST['supp'])) {
  ...
  $req=$db->prepare("DELETE FROM quartier WHERE id_quartier=:id_quartier");
...
<tbody>
<?php $requete=$db->query("SELECT * FROM quartier INNER JOIN commune ON quartier.ref_commune=commune.id_com"); 
...
<div class="modal fade" id="large-Modal<?= $g['id_quartier']; ?>" tabindex="-1" role="dialog">
...
<div class="modal-body">
   <form action="" method="post">
    <input type="hidden" name="id_quartier" value="<?= $g['id_quartier']; ?>">

 
  <div class="row">
  <div class="col-md-6">
    <?php $repp=$db->query("SELECT * FROM commune"); ?>
        <select class="form-control" name="ref_commune" id="">
                                    <?php while ($ggg = $repp->fetch()) { ?>
                                <option value="<?= $ggg['id_com'];?>"><?= $ggg['nom_com'];?> </option>
                                <?php } ?>
...
<div class="modal fade" id="large-Mod<?= $g['id_quartier']; ?>" tabindex="-1" role="dialog">
<div class="modal-dialog modal-lg" role="document">
<div class="modal-content">
<div class="modal-header">
<h4 class="modal-title">Suppression du quartier quartier <?= $g['nom_quartier']; ?> </h4>
<button type="button" class="close" data-dismiss="modal" aria-label="Close">
<span aria-hidden="true">&times;</span>
</button>
</div>
<div class="modal-body">
  ...
  <div class="row">
     ...
     <div class="col-md-12">
        <h4>Voulez vous supprimer le quartier <?= $g['nom_quartier']; ?>?</h4>
     </div>
...
<div class="modal-footer">
<button type="button" class="btn btn-danger waves-effect " data-dismiss="modal">Fermer</button>
<button type="submit" name="supp" class="btn btn-success waves-effect waves-light ">Modifier</button>
</div>
</form>
</div>
</div>
...
<td>
<button type="button" class="btn btn-warning waves-effect" data-toggle="modal" data-target="#large-Modal<?= $g['id_quartier']; ?>"><i class="feather icon-edit"></i></button>
<button type="button" class="btn btn-danger waves-effect" data-toggle="modal" data-target="#large-Mod<?= $g['id_quartier']; ?>"><i class="feather icon-trash"></i></button>
</td>
...
<div class="modal-body">
  ...
  <div class="row">
    <div class="col-md-6">
    <?php $rep=$db->query("SELECT * FROM commune"); ?>
        <select class="form-control" name="ref_commune" id="">
       
            <option value="">--Commune--</option>
                                    <?php while ($gg = $rep->fetch()) { ?>
                                <option value="<?= $gg['id_com'];?>"><?= $gg['nom_com'];?> </option>
                                <?php } ?>
     ...
     <div class="col-md-6">
        <input class="form-control form-control-round" name="nom_quartier" type="text" placeholder="Nom du quartier">
     </div>
